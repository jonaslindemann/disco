...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec2 muv1 = vec2(floor(uv.x * 2.0) / 2.0, floor(uv.y * 10.0) / 10.0) + iTime * 0.01;
    vec2 muv2 = vec2(floor(uv.x * 4.0) / 4.0, floor(uv.y * 16.0) / 16.0) + iTime * 0.98;
    vec2 muv3 = vec2(floor(uv.x * 8.0) / 10.0, floor(uv.y * 14.0) / 14.0) + iTime * 0.5;
	...
	vec4 texture = vec4(texture(iChannel0, mergeUv));
